...
def packetCheck(data, deviceEUI):
    ...
    if "nonce" in data.keys():
        ...
        nonceCheck = Recording.query.filter(Recording.date_time.between(datetime.datetime.now() - datetime.timedelta(days=1), datetime.datetime.now), Recording.sensor.equals(sensor), Recording.nonce.equals(nonce))
        if len(nonceCheck) == 0:
            nonceValid = True
