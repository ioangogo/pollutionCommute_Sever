...
def packetCheck(data, deviceEUI):
    ...
    if "nonce" in data.keys():
        ...
        nonceCheck = Recording.query.filter(Recording.date_time.between(datetime.datetime.now() - datetime.timedelta(days=7), datetime.datetime.now()), Recording.sensor == sensor, Recording.nonce == nonce)
        print(nonceCheck)
        print(nonce)
        print(deviceEUI)
